...
<head>
    ...
    <script>
        ...
        function cutPage(pn){
        	$.post("/ChinaLife/email/findByItem.do","pageNo="+pn+"&"+$("#f1").serialize(),function(info){
        		var str = "";
        		for(var i = 0; i < info.objects.length; i++){
        			var obj = info.objects[i];
        			str += "<tr><td><a href='/ChinaLife/email/findById.do?id="+obj.id+"'>"+obj.title+"</a></td><td>"+obj.state+"</td><td>"+obj.email.senderName+"</td>"+
        			"<td>"+obj.email.emailTime+"</td><td><a href='/ChinaLife/email/del.do?id="+obj.id+"'>删除</a></td></tr>";
        		}
        		$("#data").html(str);
        		if(pn == 1){
